...
async function editCepa(req, res, data, id) {
    const { cepa_id, nombre, origen, medio } = data; // Obtener los nuevos datos de la cepa desde la solicitud
    ...
    try {
        ...
        await connection.query(editCepaScript, [nombre, origen, medio, cepa_id]);
        res.status(200).json({ message: 'Cepa editada correctamente', cepa_id: cepa_id });
    } catch (error) {
...
async function getAllCepas(req, res) {
    const getAllCepasScript = 'SELECT * FROM cepas';
    
    try {
        // Ejecutar la consulta para obtener todas las cepas de la tabla "cepas"
        const result = await connection.query(getAllCepasScript);
        if (result.rows.length > 0) {
